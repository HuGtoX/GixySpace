...
/**
 * GET /api/weather/daily - 获取每日天气预报（包含实时天气、空气质量和7天预报）
 *
 * 查询参数:
 * - location: LocationID或经纬度坐标（可选，如果不提供则根据IP自动获取）
 * - lang: 多语言设置（可选，默认zh）
 * - unit: 数据单位设置（可选，默认m）
 * - days: 预报天数（可选，默认7天，最大15天）
 *
 * 示例:
 * - /api/weather/daily?location=101010100
 * - /api/weather/daily?location=116.41,39.92&days=7
 * - /api/weather/daily （自动根据IP获取位置）
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const days = Math.min(parseInt(searchParams.get("days") || "7"), 15);
    ...
    const [weatherRes, airRes, forecastRes] = await Promise.all([
      ...
      fetch(`${baseUrl}/v7/weather/${days}d?${forecastParams.toString()}`),
    ]);
    ...
    console.error("Weather Daily API error:", error);
